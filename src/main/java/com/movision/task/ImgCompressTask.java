 ...
 * 注意点：数据库中存的图片的url需要把域名替换成测试服务器的IP，把image.mofo.pics.com转化成139.196.189.100
 *
 * 2开发环境，同测试环境 没有从数据库中取数据。
 *
 * 3 生产环境， 先从文件服务器下载图片到WEB服务器， 存入临时文件夹， 然后压缩到临时文件夹，
 * 最后上传到文件服务器指定文件夹。 注意点：数据库中存的图片的url不需要更改域名，保持不变，即image.mofo.pics.com
 * 
 * @author zhuangyuhao
...
public class ImgCompressTask {
    ...
    // 邮件接受者 "zhuangyuhao@mofo.pics.com"
    private static final String[] mailTo = {"zhuangyuhao@mofo.pics.com"};
    // 当前图片上传模式
